-- WB-Dashboard.lua
-- Version 1.6: 2020-06-19
--              Correction of maxEGTchanged/maxRPMchanged update bug

local wVersion="1.6"
local wAppname="Whitebox Dashboard"

-- Locals for the application

local wBrand="whitebox"
local wbRPMParam = 2
local wbEGTParam = 3
local wbFuelParam = 1
local wbBattParam = 5
...
local function DrawRpmGauge(iRPM, size)
    ...
    if (iRPM > maxRPM * 1000 and iRPM ~= nil) then
...
local function DrawEgtGauge(iEGT, size)
    ...
    if (iEGT > maxEGT and iEGT ~= nil) then
        maxEGTChanged(iEGT)
    end
